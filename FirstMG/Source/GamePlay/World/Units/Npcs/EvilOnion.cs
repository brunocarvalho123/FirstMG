...
namespace FirstMG.Source.GamePlay
{
    class EvilOnion : Npc
    {
        ...
        private MyTimer _moveTimer    = new MyTimer(500);
        private MyTimer _shooterTimer = new MyTimer(1000);

        public EvilOnion(Vector2 a_position, Vector2 a_frames) 
            : base("Assets\\evil_onion", a_position, new Vector2(50,50), a_frames)
        {
            MovSpeed = 2f;
            MaxHSpeed = 4f;
        }
        ...
        public override void Update(Vector2 a_offset, MainChar a_mainChar, SquareGrid a_grid)
        {
            ...
            {
                if (Globals.GetDistance(Position, a_mainChar.Position) < 300)
                {
